...
class OpenStreetMapMigrationService(DataMigrationService):
    ...
    async def fetch_data(self, args):
        ...
        async def step(min_lat_loc, max_lat_loc, min_lon_loc, max_lon_loc):
            filter_query = sync_to_async(OpenTripMapServiceData.objects.filter)(
                min_latitude=min_lat_loc,
                max_latitude=max_lat_loc,
                min_longitude=min_lon_loc,
                max_longitude=max_lon_loc
            )
            was_processed = await filter_query

            if await sync_to_async(was_processed.exists)():
            ...
            self.logger.info(
                f'Found {len(places_ids)} places for step with lat: {min_lat}-{max_lat} and lon: {min_lon}-{max_lon}'
            )

            async def fetch_place_data(place_id):
                place_data = self.api_service.request(
                    method='GET',
                    endpoint=f'/places/xid/{place_id}',
                    query_params=dict(apikey=self.api_key)
                )
                # print(f"Fetched place data for {place_id}")
                return place_data

            place_tasks = [fetch_place_data(place_id) for place_id in places_ids]

            await sync_to_async(OpenTripMapServiceData.objects.create)(
                min_latitude=min_lat_loc,
                max_latitude=max_lat_loc,
                min_longitude=min_lon_loc,
                max_longitude=max_lon_loc
        ...
        step_tasks = [step(lat, lat + step_lat, lon, lon + step_lon) for lat in np.arange(min_lat, max_lat, step_lat) for lon in np.arange(min_lon, max_lon, step_lon)]
